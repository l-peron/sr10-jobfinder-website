...
router.get('/user/:id/desactivate', function(req, res, next) {
  ...
  userModel.setActive(user_id, 0, function(result) {
    return res.redirect('/admin/user/list');
  })
})
...
router.get('/user/:id/activate', function(req, res, next) {
  ...
  userModel.setActive(user_id, 1, function(result) {
    return res.redirect('/admin/user/list');
  })
})
...
router.get('/organization/:siren/accept', function(req, res, next) {
  ...
  organizationModel.setStatus(org_siren, 'accepted', function(result) {
...
router.get('/organization/:siren/reject', function(req, res, next) {
  ...
  organizationModel.setStatus(org_siren, 'refused', function(result) {
    ...
    organizationModel.getOrganizationCreator(org_siren, function(result) {
      ...
      organizationMembersModel.delete(org_siren, user_id, function(result) {
        return res.redirect('back');
      })
    })
  })
